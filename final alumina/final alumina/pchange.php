...
// $x="UPDATE  `sslc` SET  `board` =  '$board' , `reg_or_sup` =  '$reg', `pass_year` =  '$pass' WHERE  `user`.`username` ='$user' LIMIT 1 ";
    ...
    $x="UPDATE  `principal` SET `password` = '$user'
       WHERE  `name` ='$u' LIMIT 1" ;
    $res =  mysqli_query($db,$x);
    // header('location:udash.php');
    // }
    // else{ echo "error"; }    
    echo '<script language="javascript">document.location.href="princi.php?ans=Updated Your Password Sucessfully.."</script>';
}
